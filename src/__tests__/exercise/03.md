...
1. To avoid implementation details- look for items the way end user will find on
   UI. For e.g. look for button by it's name (by accessibility) and not by it's
   position in dom, so that your test won't break even if implementation changes
