...
1. Refer server and server-handlers in test dir to learn about how to mock
   server using msw for testing and developing time when apis are not ready.
2. Use waitForElementToBeRemoved when waiting for loading to disappear
3. Use toMatchInlineSnapshot() when you think the text content to be tested
   could change, so that using this function will give you an option to update
   the text automatically
4. Use server.use(handler) when testing specific case and not cluttering
   application-wide/generic handlers
